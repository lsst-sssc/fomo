...
None — all v2.1 target features shipped (Phases 18-22; Phase 22 added 2026-07-14 to close the Phase 21 site-matching functionality gap, now complete 2026-07-15). Milestone close via `/gsd-complete-milestone` is the next step, not yet run.
...
---
*Last updated: 2026-07-17 — Phase 23 (weather/storm cancellation handling) complete: classical-schedule `[CANCELLED]` title prefix, staff-facing Mark Cancelled/Mark Weathered action on approved `CampaignRun`s from the approval queue's Decided table with in-place, existence-guarded calendar sync, and a `[WEATHERED]` terminal box-shadow ring. 3/3 plans executed; the real Gemini FT-115 program proven to flow through the same mechanism with no special-casing. Code review clean (0 critical); phase verification 15/15 must-haves passed. 536/536 tests pass. This was an organic phase outside v2.1's original REQ scope (triggered by a real storm), layered on top of the already-shipped v2.1 Uncertain Scheduling & Site Disambiguation milestone; milestone close (`/gsd-complete-milestone`) still not yet run.*
