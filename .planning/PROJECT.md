...
All originally-scoped v2.1 target features shipped (Phases 18-22; Phase 22 added 2026-07-14 to close the Phase 21 site-matching functionality gap, now complete 2026-07-15). Two organic phases were layered on afterward — Phase 23 (weather/storm cancellation handling, complete 2026-07-17) and Phase 25 (range-window CalendarEvent projection fix, complete 2026-07-18) — plus Phase 24 (operator/usage runbook documentation), which is still `TBD` (never planned) despite being numbered ahead of Phase 25 in ROADMAP.md. Milestone close via `/gsd-complete-milestone` is blocked until Phase 24 is planned and executed (or explicitly descoped) — not yet run.
...
---
*Last updated: 2026-07-18 — Phase 24 (operator and usage runbook documentation) complete: published a consolidated, task-oriented operator runbook (`docs/runbooks/telescope_runs_calendar.rst`) covering all five telescope-runs-calendar management commands plus the approval-queue staff actions, a manage.py onboarding subsection in `docs/installation.rst`, and the toctree wiring in `docs/index.rst`. Docs-only, no source behavior change. 1/1 plan executed; code review clean (0 critical, 1 warning, 2 info); phase verification 11/11 must-haves passed; regression gate 359/359 prior-phase tests pass. This satisfies Phase 25's declared `Depends on: Phase 24`. **Milestone v2.1 is now complete** — all phases (18-25) shipped; `/gsd-complete-milestone` can be run.*
