...
**Note — milestone v2.1 is NOT yet complete:** Phase 25 declares `Depends on: Phase 24` in ROADMAP.md, but Phase 24 (operator/usage runbook documentation) has never been planned (0 plans, still `TBD`) — Phase 25 was executed out of its own declared dependency order because it was a real, time-sensitive bug affecting live campaign data. `/gsd-complete-milestone` must NOT be run until Phase 24 is planned and executed (or explicitly descoped).
...
---
*Last updated: 2026-07-18 — Phase 25 (range-window CalendarEvent projection) complete: `_project_calendar_event()` now projects one dip-corrected `CalendarEvent` per night for approved, site-resolved range-window `CampaignRun`s (ground runs; satellite runs unchanged), `_set_run_status()` syncs every per-night event on cancel/weather, and a new `backfill_range_calendar_events` management command retroactively projects events for already-approved runs. 2/2 plans executed; code review clean (0 critical, 7 warnings); phase verification 15/16 auto-verified + 1 human-verified (real backfill run against live dev DB confirmed CampaignRun pk=34 — the motivating GS-2026A-FT-115 case — now has its 4 events); security review 5/5 threats closed. 544/544 tests pass. This was an organic gap-closure phase (from `/gsd-debug`) layered on top of the already-shipped v2.1 Uncertain Scheduling & Site Disambiguation milestone. **Milestone v2.1 is still NOT complete**: Phase 25 declares `Depends on: Phase 24` in ROADMAP.md, but Phase 24 (operator/usage runbook documentation) has never been planned — `/gsd-complete-milestone` must not run until Phase 24 is resolved.*
