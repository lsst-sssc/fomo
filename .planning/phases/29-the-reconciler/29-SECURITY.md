...
| `CampaignRun` row state -> projection decision | `source` / `telescope_class` / `site` / window drive which key family is written; no HTTP input crosses here. | Staff-editable model fields (admin-gated; `source` withheld on `web` rows) |
...
| T-29-07 | Tampering | Reconciler overwriting an `ObservationRecord`-derived event | high | mitigate | Queue runs dispatch to the container branch only (`campaign_reconciler.py:430-433`, `QUEUE_SOURCES` at `:75`); the per-night branch is unreachable for them. `TestQueueOwnershipDoesNotTouchRecordEvents` (`tests/test_campaign_reconciler.py:554-618`) asserts the LCO-portal-keyed record event's `url`/`title`/`start`/`end` and `modified` are unchanged after **two** reconcile passes (`:615-618`), and that `owned_events(run)` returns exactly the one bare-key row. | closed |
