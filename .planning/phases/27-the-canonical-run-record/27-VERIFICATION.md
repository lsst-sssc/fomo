...
| `solsys_code/management/commands/repair_stale_campaign_run_sites.py` | 193-199 | Never clears `telescope_class` when it resolves `site` | WARNING (carried from 27-REVIEW.md WR-02) | Could produce a row with both a real site and a stale `telescope_class`, violating the model's own documented invariant. Untested interaction. Did not manifest on the live dev DB only due to plan ordering (repair ran before the `telescope_class` field existed). |
