...
- [ ] 35-21-PLAN.md — CR-02: the provenance token carries every mint input (version, site, sub-night pair), a pre-release token reads as unrecorded, the column widens to 64 (IN-02 rider) — plus the round's paired docs: runbook `retired`/`detach_declined` corrections, a post-upgrade deploy note, and `reconcile_campaign_runs_demo.ipynb` re-executed (wave 2)
