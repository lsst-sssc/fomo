...
- [ ] 35-23-PLAN.md — CR-04 + CR-05 + WR-06 + IN-05: decline only the destructive half of a re-mint so a declined night still gets its label refresh, guard the retirement branch's own allocation-night delete, and split `detach_declined` into two counters so each operator message is true (wave 1, tracer)
