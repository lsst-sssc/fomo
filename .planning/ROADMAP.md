...
- [ ] 35-20-PLAN.md — CR-01 + CR-03: a human-confirmation and staff-state guard before the re-mint delete, then compute-before-destroy plus a `transaction.atomic()` boundary so a failed re-mint cannot leave a hole (wave 1, tracer)
- [ ] 35-21-PLAN.md — CR-02: the provenance token carries every mint input (version, site, sub-night pair), a pre-release token reads as unrecorded, the column widens to 64 (IN-02 rider) (wave 2)
