...
- [x] **CANON-01**: A `CampaignRun` records which ingest path created it (`source`: web submission, classical file, LCO queue, Gemini queue, CSV import), and approval is required only for web submissions
...
- [x] **ATTRIB-01**: Staff see a queue of suggested associations between existing calendar events or observation records and their likely parent run, showing the evidence for each candidate (matched telescope, date overlap, campaign, instrument-string similarity)
...
- [x] **ATTRIB-05**: The known real case is surfaced as a candidate — `CampaignRun` pk=1 (FTS/MuSCAT4, 7–21 July, Siding Spring E10) and its 11 LCO queue events (`2m0`/`2M0-SCICAM-MUSCAT`, 7–20 July), which differ in both date span and instrument string
- [x] **ATTRIB-06**: Attribution can be completed before the first full reconcile sweep, so the calendar never visibly double-books
...
| CANON-01 | Phase 27 — The Canonical Run Record | Complete |
...
| ATTRIB-01 | Phase 28 — Operator-Assisted Attribution | Complete |
