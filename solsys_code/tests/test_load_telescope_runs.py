...
class TestLoadTelescopeRuns(TestCase):
    ...
    def _write_schedule_file(self, lines: list[str]) -> str:
        """Write a schedule file to a temporary path and return the path."""
        tmpdir = tempfile.mkdtemp()
        path = pathlib.Path(tmpdir) / 'schedule.txt'
        path.write_text('\n'.join(lines) + '\n')
        return str(path)
    ...
    def test_creates_one_event_per_night(self):
        ...
        path = self._write_schedule_file(['NTT EFOSC2 allocation 9-13 July'])
        try:
    ...
    def test_unchanged_rerun_does_not_update_existing_rows(self):
        ...
        try:
            ...
            # Capture modified timestamps after the first run
    ...
    def test_unparseable_line_logged_and_skipped(self):
        ...
        path = self._write_schedule_file(
            [
