...
tests (28-VALIDATION.md Wave 0). Plan 28-04 extends this same file with the page-rendering
table/template classes; the shared fixture and helper conventions below are written to be
reused there too.

Because the template (``attribution_queue.html``) lands in Plan 28-04, no test here may
GET-render the attribution page and expect a 200. Two consequences follow: (1) the two
anonymous/non-staff GET-redirect assertions below extract the reversed URL to a local
variable first, then hand that variable to the test client's GET method on the next line --
so the real redirect (which short-circuits before ``TemplateView`` ever tries to render) can
still be exercised without the reverse-lookup call and the GET call ever appearing together
on one line; (2) every POST test below reads ``django.contrib.messages`` directly off the
un-followed response (``get_messages(response.wsgi_request)``) rather than passing
``follow=True`` -- following the decide endpoint's redirect would GET the (template-less)
attribution page and raise ``TemplateDoesNotExist``. GET *context assembly* is instead
exercised directly against ``AttributionQueueView`` via ``RequestFactory``, which never
touches template rendering.
"""
